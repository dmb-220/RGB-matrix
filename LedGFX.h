...
const uint16_t  MATRIX_CENTER_X = MATRIX_WIDTH / 2;
const uint16_t  MATRIX_CENTER_Y = MATRIX_HEIGHT / 2;
...
uint16_t XY( uint16_t x, uint8_t y) 
{
  return XY16(x, y);
}
...
class LedGFX : public Adafruit_GFX {
public:
  CRGB *leds;
  //CRGB leds[NUM_LEDS];
  LedGFX() : Adafruit_GFX(MATRIX_WIDTH, MATRIX_HEIGHT){
  ...
  void drawBackgroundFastLEDPixelCRGB(int16_t x, int16_t y, uint16_t color)
  {
     uint8_t r,g,b;
  ...
  void Pixel(int16_t x, int16_t y, uint8_t colorIndex) {
...
*/
  ...
  void Caleidoscope5() {
    for (uint16_t x = 0; x < MATRIX_WIDTH / 4; x++) {
      for (int y = 0; y <= x && y<=MATRIX_HEIGHT; y++) {
        leds[XY16(x, y)] = leds[XY16(y, x)];
      }
    }

    for (uint16_t x = MATRIX_WIDTH / 4; x < MATRIX_WIDTH / 2; x++) {
  ...
  void MoveDown() {
    for (uint16_t y = MATRIX_HEIGHT - 1; y > 0; y--) {
...
/*inline void LedGFX::color565to888(const uint16_t color, uint8_t &r, uint8_t &g, uint8_t &b){
  r = ((((color >> 11) & 0x1F) * 527) + 23) >> 6;
  g = ((((color >> 5) & 0x3F) * 259) + 33) >> 6;
  b = (((color & 0x1F) * 527) + 23) >> 6;
}*/

inline void LedGFX::drawPixel(int16_t x, int16_t y, uint16_t color) // adafruit virtual void override
{
  LedGFX::drawBackgroundFastLEDPixelCRGB( x, y, color);
} 

inline void LedGFX::fillScreen(uint16_t color)  // adafruit virtual void override
{
  for (int y=0; y<MATRIX_HEIGHT; ++y){
      for (uint16_t x = 0; x < MATRIX_WIDTH; ++x){
      LedGFX::drawBackgroundFastLEDPixelCRGB( x, y, color);
